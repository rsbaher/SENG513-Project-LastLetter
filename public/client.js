...
// When window loads, let user sign in or redirect them to home page
    window.onload = function () {
        ...
        initProfile();
        initUserAuth();
        loadLoginPage();        // TODO I think the problem is here when you go back from the different website to game
        initLeaderboard();
        ...
        // TODO I had to place it here so that it works properly!!!!!!!!!!!!!!!!!!!
        // TODO I do not know how to fix it :( very sad
        $('form').submit(function() {
            socket.emit('single-player-input', $('#single-player-input').val());
            $('#single-player-input').val('');
            return false;
...
// Server is sending a leaderboard entry
    socket.on('get leaderboard', function (single, leaderboardEntry) {
        ...
        if (single) {
            $('#singleplayer-leaderboard').append('<li>' + userName + ': ' + userScoreSingle + '</li>');
        }
        else {
            $('#multiplayer-leaderboard').append('<li>' + userName + ': ' + userScoreMulti + '</li>');
        }
    });
